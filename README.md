...
| 功能 | 模块 | 说明 |
| --- | --- | --- |
| 子图匹配 | `src/subgraph_matching/` | 训练图编码器并判断两图是否存在子图关系 |
| 频繁子图挖掘 | `src/subgraph_mining/` | 复用匹配模型，对候选 pattern 做搜索与筛选 |
| 结果分析 | `src/analyze/` | 分析 embedding、统计 pattern 频次、可视化结果 |
...
src/
   core/
      ...
      data.py               数据源：OTFSynDataSource / DiskDataSource
      ...
      combined_syn.py       合成图生成器（ER / WS 等）
      cli.py                运行时参数设置
      ...
      io/                   图 I/O
         ...
         graph_io.py
      sampling/             邻域采样
         neighborhood.py     BFS 前沿扩展采样
         enumeration.py      ESU 子图枚举
         baseline_queries.py 基线查询生成

   subgraph_matching/       阶段 1：训练子图关系编码器
      config.py             子图匹配训练参数
      train.py              训练入口（train / train_step / train_loop）
      test.py               评估与验证入口（validation / compute_metrics / plot_pr_curve）
      alignment.py          图对齐与匹配分数工具

   subgraph_mining/         阶段 2：频繁子图挖掘
      config.py             挖掘阶段参数
      decoder.py            SPMiner 挖掘入口（薄包装）
      pipeline.py           PatternGrowthPipeline 流水线类
      search_agents.py      GreedySearchAgent / MCTSSearchAgent

   analyze/
      count_patterns.py     模式频次统计
      analyze_pattern_counts.py 统计结果分析

   compare/
      compare.py            基线比较入口
      analysis.py           比较分析函数
      benchmarking.py       gSpan / SPMiner 运行封装
      plotting.py           可视化
      visualize_mined_subgraphs.py
...
```bash
python -m src.subgraph_matching.train --node_anchored
```

使用真实数据训练，以 Facebook 为例：

```bash
python -m src.subgraph_matching.train --dataset=facebook --node_anchored --output_strategy version
```

### 2. 评估模型

```bash
python -m src.subgraph_matching.test --node_anchored --output_strategy version
```
...
```bash
python -m src.subgraph_mining.decoder --dataset=enzymes --node_anchored --output_strategy version
```

以 Facebook 为例：

```bash
python -m src.subgraph_mining.decoder --dataset=facebook --node_anchored --output_strategy version
```

## 统一产物管理

为避免结果覆盖和目录散乱，项目已接入统一输出策略。

### 运行时输出参数

| 参数 | 默认值 | 说明 |
| --- | --- | --- |
| `--output_root` | `results` | 统一产物根目录 |
| `--output_strategy` | `version` | 输出冲突策略：`version` 自动版本化，`overwrite` 直接覆盖 |
| `--output_tag` | 空 | 产物目录附加标签（用于区分实验批次） |

### 默认输出目录结构

```text
results/
   matching/<dataset>/<run_name>/
   mining/<dataset>/<run_name>/
   count/<dataset>/<run_name>/
   analyze/<dataset>/<run_name>/
   alignment/<dataset>/<run_name>/
```

每个任务目录通常包含核心产物文件、图像文件和 `manifest.json`（记录运行参数及输入/输出路径）。

### 覆盖策略说明

- `--output_strategy version`：遇到同名文件时自动添加时间戳/序号后缀。
- `--output_strategy overwrite`：直接覆盖目标文件。
...
训练脚本位于 [src/subgraph_matching/train.py](src/subgraph_matching/train.py)。最常用的参数如下：
...
挖掘脚本位于 [src/subgraph_mining/decoder.py](src/subgraph_mining/decoder.py)。它会：

1. 读取训练好的 checkpoint。
2. 从目标数据集中采样邻域或整图。
3. 对候选子图做 batch embedding。
4. 使用 Greedy 或 MCTS 搜索模式。
5. 将结果保存到统一任务目录（默认位于 `results/mining/<dataset>/<run_name>/`）。
...
| `--out_path` | `results/out-patterns.p` | 输出结果文件（默认值会映射到统一任务目录） |
...
```bash
jupyter notebook docs/notebooks/Analyze\ Embeddings.ipynb
```
...
```bash
python -m src.analyze.count_patterns --dataset=enzymes --out_path=results/counts.json --node_anchored
```

以 Facebook 为例：

```bash
python -m src.analyze.count_patterns --dataset=facebook --out_path=results/counts.json --node_anchored
```
...
当前仓库以 CLI 工作流为主，推荐通过 `python -m ...` 运行各阶段任务。
...
```bash
python -m src.subgraph_matching.train --dataset=facebook --node_anchored --n_batches 20 --eval_interval 10 --batch_size 32 --n_workers 1 --model_path results/facebook_train_big.pt --output_strategy version
```

### 挖掘示例

```bash
python -u -m src.subgraph_mining.decoder --dataset=facebook --node_anchored --model_path results/facebook_train_big.pt --n_neighborhoods 50 --batch_size 50 --n_trials 5 --out_path results/facebook_patterns_big.p --output_strategy version
```
...
1. 先看 [src/core/models.py](src/core/models.py) 和 [src/core/data.py](src/core/data.py)，理解模型与数据流。
2. 再看 [src/subgraph_matching/train.py](src/subgraph_matching/train.py) 和 [src/subgraph_matching/test.py](src/subgraph_matching/test.py)，理解匹配任务。
3. 最后看 [src/subgraph_mining/pipeline.py](src/subgraph_mining/pipeline.py) 和 [src/subgraph_mining/search_agents.py](src/subgraph_mining/search_agents.py)，理解挖掘流程。

## 重构说明

项目已完成模块化解耦重构，主要改动：

- **utils.py 拆分** — 原 15+ 函数按职责拆分为 `device.py`、`hashing.py`、`optimizer.py`、`batch.py`、`io/` 和 `sampling/` 子模块，`from src.core import utils` 通过 shim 保持向后兼容。
- **配置数据类** — `src/core/config.py` 提供 `MatchingConfig`、`MiningConfig`、`RuntimeConfig`、`AugmentConfig` 四个 dataclass，支持 `from_namespace()` 工厂方法。
- **流水线类** — `src/subgraph_mining/pipeline.py` 将原 140 行 `pattern_growth()` 抽取为 `PatternGrowthPipeline`，采样、编码、搜索、可视化、序列化为独立方法。
- **数据集注册表** — `load_dataset_for_stage()` 的 if-elif 链替换为 `_LOADER_REGISTRY` 字典，新增数据集只需注册加载器。
- **函数拆分** — `validation()` 拆出 `compute_metrics()`、`plot_pr_curve()`、`log_checkpoint_metrics()`；`train()` 拆出 `train_step()`。
- **BFS 去重** — `frontier_sample_nodes()` 统一 BFS 前沿扩展逻辑，`data.py` 和 `sample_neigh` 共用。
