...
```bash
python -m src.subgraph_matching.train --dataset=facebook --node_anchored
```

### 2. 评估模型

```bash
python -m src.subgraph_matching.test --node_anchored
```
...
```bash
python -m src.subgraph_mining.decoder --dataset=enzymes --node_anchored
```

以 Facebook 为例：

```bash
python -m src.subgraph_mining.decoder --dataset=facebook --node_anchored
```
...
```bash
python -m src.subgraph_matching.train --dataset=facebook --node_anchored --n_batches 20 --eval_interval 10 --batch_size 32 --n_workers 1 --model_path results/facebook_train_big.pt
```

### 挖掘示例

```bash
python -u -m src.subgraph_mining.decoder --dataset=facebook --node_anchored --model_path results/facebook_train_big.pt --n_neighborhoods 50 --batch_size 50 --n_trials 5 --out_path results/facebook_patterns_big.p
```
...
3. 最后看 [src/subgraph_mining/decoder.py](src/subgraph_mining/decoder.py) 和 [src/subgraph_mining/search_agents.py](src/subgraph_mining/search_agents.py)，理解挖掘流程。
