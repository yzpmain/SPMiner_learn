...
Supported real-world datasets include TUDataset benchmarks (`enzymes`, `proteins`, `cox2`, `aids`, `reddit-binary`, `imdb-binary`, `dblp`, `firstmm_db`) and the **Stanford SNAP ego-Facebook** social network (`facebook`).
...
2. To mine patterns from the Stanford Facebook network: `python3 -m subgraph_mining.decoder --dataset=facebook --node_anchored`
   (requires `data/facebook_combined.txt` from https://snap.stanford.edu/data/ego-Facebook.html)
...
- Count patterns on the Facebook dataset: `python3 -m analyze.count_patterns --dataset=facebook --out_path=results/counts.json --node_anchored`
